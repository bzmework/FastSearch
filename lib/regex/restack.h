...
template< bool f > struct static_asserts;
template<>         struct static_asserts<true> { static_asserts() {} };
...
private:
    ...
    union
    {
		...
		byte_t      m_buf[((sizeof(stack_node::header) + sizeof(void*) - 1) & ~(sizeof(void*) - 1)) + StaticBlockSizeT];
    } m_first_node;
...
public:
    ...
    ~hetero_stack() // throw()
    {
        ...
        detail::static_asserts< detail::is_power_of_two<AlignmentT>::value > const align_test;
    ...
    inline void push( T const & t ) // throw(std::bad_alloc,...)
    {
        ...
        detail::static_asserts<( AlignmentT >= detail::alignofs<T>::value )> const align_test;
    ...
    {
        detail::static_asserts< RuntimeTypeCheckT > const type_check;
        static_cast<void>(type_check);
