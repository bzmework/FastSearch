/*
模块： c2int64.c
功能： 转换字符串为int64(long long)类型值。
参数： str，要转换的字符串。
返回： 返回int64(long long)值。
说明： c2int64()与标准函数strtoll功能一致。
	  c2int64()会扫描参数str字符串，跳过前面的空格字符，直到遇上数字或正负符号才开始做转换，
...
int64 c2int64(const tchar* str)
{
	...
	while (_c_isdigit(c))
	{
		if (_int64_overflow(number, c)) return negative ? _LLONG_MIN : _LLONG_MAX; //溢出
		number = number * 10 + _c_tonum((int64)c); //数字累加
		c = (int16) * (++p); //获取下一个字符
	}
