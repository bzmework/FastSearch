/*
模块： c2uint64.c
功能： 转换字符串为uint64(unsigned long long)类型值。
参数： str，要转换的字符串。
返回： 返回uint64(unsigned long long)值。
说明： c2uint64()与标准函数strtoull功能一致。
	  c2uint64()会扫描参数str字符串，跳过前面的空格字符，直到遇上数字或正负符号才开始做转换，
...
uint64 c2uint64(const tchar* str)
{
	...
	while (_c_isdigit(c))
	{
		if (_uint64_overflow(number, c)) return _ULLONG_MAX; //溢出
		number = number * 10 + _c_tonum((uint64)c); //数字累加
		c = (int16) * (++p); //获取下一个字符
	}
